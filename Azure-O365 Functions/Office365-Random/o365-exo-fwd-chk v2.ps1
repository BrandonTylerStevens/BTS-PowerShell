  ...
  foreach ($rule in $rules)
    {
       If ($rule.enabled) {
        ...
        if ($rule.forwardto -or $rule.RedirectTo -or $rule.DeleteMessage -or $rule.ForwardAsAttachmentTo -or $rule.SendTextMessageNotificationTo) { write-host -ForegroundColor $warnmessagecolor "    Suspect Enabled Rule name -",$rule.name }
